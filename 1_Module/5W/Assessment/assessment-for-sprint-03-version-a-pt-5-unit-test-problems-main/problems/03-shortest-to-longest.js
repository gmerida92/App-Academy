...
function shortestToLongest(strings, sorted = []) {
  ...
  {
    if(strings[i].length === strings[shortestWordIndex].length)
    {
      if(strings[i] < strings[shortestWordIndex])
      {
        shortestWordIndex = i;
      }
    }
  ...
  sorted.push(strings[shortestWordIndex]);
  strings.splice(shortestWordIndex, 1);
