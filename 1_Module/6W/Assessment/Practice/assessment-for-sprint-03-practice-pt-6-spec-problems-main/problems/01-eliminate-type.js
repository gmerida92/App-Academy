...
function eliminateType(arr) {
  return (type) => {
    return arr.filter(element => {return typeof(element) !== type
...
// const eliminateType = arr => elim = (type, array = arr.slice(0)) => array.length === 0 ? [] : type.includes(typeof array[0]) ? elim(type, array.slice(1)) : [array[0], ...elim(type, array.slice(1))];
